...
<?php
    ...
    $dateAdded = "%20["."20140101000000"."%20TO%20".$endDate."]&fields=id,title,dateAdded,owner,status,submissions,sendouts,interviews,isOpen";

    $restUrl = $RT_Url->{"restUrl"};
    $queryUrl = "search/JobOrder?query=dateAdded:".$dateAdded;
    ...
    for($i = 0; $i < $loopCnt; $i++){
        $start = $i * 60;
        $dataUrl = $pageUrl."&start=".$start."&count=60";
    ...
    $restCnt = $total - $loopCnt*60;
    if($restCnt > 0){
        $dataUrl = $pageUrl."&start=".($i*60)."&count=".$restCnt;
        $required_data = curl($dataUrl);
        $json = json_decode($required_data);
        $data = $json->{"data"};
        $output_data []= $data;
    ...
    $required_data_file = fopen("../data/job.json","w");
